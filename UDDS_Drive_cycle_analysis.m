...
data=xlsread("UDDS.xlsx");
t= data(:,1);
speed= data(:,2);
v= speed*(5/18);                          % Converting in m/s
for i= 1:(length(v)-1)
    If(i)= M*(v(i+1)-v(i));               % Inertial force 
    Ip(i)= M*(v(i+1)-v(i))*v(i);          % Inertial Power
    Af(i)= 0.5*rho*A_f*C_d*(v(i))^2;      % Aerodynamic force
    Ap(i)= 0.5*rho*A_f*C_d*(v(i))^2*v(i);                        % Aerodynamic Power
    Rf(i)= C_r*M*g;                                              % Rolling force
    Rp(i)= C_r*M*g*v(i);                                         % Rolling power
